...
async def verify_command(interaction: discord.Interaction, member: discord.Member, ign: str, nation: str = None):
    ...
    try:
        ...
        if result.success:
            ...
            # Determine relationship and get appropriate county info
            relationship = mgm.determine_relationship_status(guild_nation, nation_uuid) if mgm else 'citizen'
            
            # Only get county info for citizens
            ...
            if success:
                ...
                # Create success embed with relationship info
                if is_reverification:
...
async def verify_cross_nation_command(interaction: discord.Interaction, member: discord.Member, ign: str):
    ...
    try:
        ...
        relationship = mgm.determine_relationship_status(guild_nation, player_nation_uuid)
        ...
        success = await mgm.assign_roles_for_nation(
            ...
            player_nation_uuid, guild_nation
        )
        ...
        await interaction.followup.send(embed=embed)import discord
import logging
import os
import asyncio
...
async def verify_command(interaction: discord.Interaction, member: discord.Member, ign: str, nation: str = None):
    ...
    try:
        ...
        if result.success:
            ...
            if mgm and town_uuid:
                county_name, county_role_id, has_county = mgm.get_county_for_town_in_nation(target_nation, town_uuid)
            ...
            if success:
                ...
                if mgm and not has_county:
                    county_system = mgm.get_county_system_for_nation(result.nation)
                    if county_system.get("enabled", False):
                        county_notice_embed = response_manager.create_embed(
                            "verification_county_notice",
                            town=result.town,
